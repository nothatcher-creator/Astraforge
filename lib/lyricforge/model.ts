...
export interface Asset { id:string; name:string; type:'audio'|'image'|'video'|'font'; mime:string; size:number; duration?:number; fontFamily?:string; source?:{url:string;provider:string;creator:string;license:string;licenseUrl?:string;licenseText?:string}; }
