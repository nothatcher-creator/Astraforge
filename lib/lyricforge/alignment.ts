...
export interface AlignmentOptions {windowMs:number;clipIds?:string[];range?:{start:number;end:number};expectedOffsetMs?:number;}
...
function candidates(tokens:string[],spoken:Recognized[],clip:Clip,windowMs:number){
 ...
 for(let end=1;end<=m;end++){
  ...
  const distance=(Math.abs(spoken[first.spoken].start-clip.start)+Math.abs(spoken[last.spoken].end-clip.end))/Math.max(1000,windowMs);
  const span=last.spoken-first.spoken+1,skipped=span-pairs.length;
...
export function alignExistingLyrics(project:Project,recognized:Word[],options:AlignmentOptions):AlignmentReport {
 const windowMs=clamp(Number.isFinite(options.windowMs)?options.windowMs:5000,100,120000);
 ...
 const proposals=lines.map(c=>{
  ...
  const expected={...c,start:c.start+offset,end:c.end+offset};
  const nearby=valid.filter(w=>w.start>=expected.start-windowMs&&w.end<=expected.end+windowMs);
  if(nearby.length>2000){row.reason='Narrow the search window or split this long line.';return row;}
  const choices=candidates(tokens,nearby,expected,windowMs),group:Candidate[]=[];
  for(const choice of choices.slice(0,24)){
   ...
   const alternative=choices.find(v=>v.pairs.length===pairs.length&&Math.abs(nearby[v.pairs[0].spoken].start-firstWord.start)>700&&Math.abs(Math.abs(nearby[v.pairs[0].spoken].start-expected.start)-Math.abs(firstWord.start-expected.start))<500);
   ...
   const scattered=last.spoken-first.spoken+1>pairs.length;
   const largeMove=Math.max(Math.abs(start-expected.start),Math.abs(end-expected.end))>clamp(windowMs*.5,1000,2500);
   const reliable=anchored&&coverage===1&&allExact&&tokens.length>1&&!alternative&&!largeGap&&!scattered&&!largeMove;
