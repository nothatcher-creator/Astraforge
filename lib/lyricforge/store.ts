...
export class EditorStore {
 ...
 merge(){const cs=this.project.clips.filter(c=>this.selected.includes(c.id)&&this.editable(c));if(cs.length<2||!cs.every(c=>c.trackId===cs[0].trackId&&(c.kind==='lyrics'||c.kind==='text')))return;const merged=mergeClips(cs)!,ids=new Set(cs.map(c=>c.id));this.update(p=>({...p,clips:p.clips.flatMap(c=>c.id===cs[0].id?[merged]:ids.has(c.id)?[]:[c])}));this.select([merged.id]);}
